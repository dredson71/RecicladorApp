...
public class WeekFragment extends Fragment {
    ...
    public void getDataWeek() {
        ...
        while (CondominiosDB.moveToNext()) {
            Cursor f = db.rawQuery("select cantidad ,peso  from Contador where tendenciaTipo = 'bolsasWeek/' and productoTipo = 'Plastico' and nombrecondominio = " + CondominiosDB.getInt(0), null);
            ...
            Cursor f3 = db.rawQuery("select cantidad ,peso  from Contador where tendenciaTipo = 'bolsasWeek/' and productoTipo = 'Papel' and nombrecondominio =  " + CondominiosDB.getInt(0), null);
            if (f3.moveToFirst()) {
    ...
    private ArrayList<Entry> dataResiduosValue()
    {
        ...
        Cursor CondominiosDB = db.rawQuery("select codigo from Condominio",null);
        while (CondominiosDB.moveToNext()) {
            Cursor f = db.rawQuery("select lunes,martes,miercoles,jueves,viernes,sabado,domingo from DatosDiarios where tipo = 'residuos' and frecuenciatipo = 'bolsasWeek/' and tipocondominio ="+CondominiosDB.getInt(0), null);
            if(f.moveToFirst()) {
                dias[0] += f.getInt(0);
                dias[1] += f.getInt(1);
                dias[2] += f.getInt(2);
    ...
    private ArrayList<Entry> dataBolsasValue()
    {
        ...
        Cursor CondominiosDB = db.rawQuery("select codigo from Condominio",null);
        while (CondominiosDB.moveToNext()) {
            Cursor f = db.rawQuery("select lunes,martes,miercoles,jueves,viernes,sabado,domingo from DatosDiarios where tipo = 'bolsas' and frecuenciatipo = 'bolsasWeek/' and tipocondominio ="+CondominiosDB.getInt(0), null);
            if(f.moveToFirst()) {
                dias[0] += f.getInt(0);
                dias[1] += f.getInt(1);
                dias[2] += f.getInt(2);
